...
namespace Langue
{
    public static class CoreOperators
    {
        ...
        public static Pattern<TResult, TContext> SelectMany<T, TIntermediate, TResult, TContext>(this Pattern<T, TContext> self,
        ...
        public static Pattern<IEnumerable<T>, TContext> Optional<T, TContext>(this Pattern<IEnumerable<T>, TContext> self) => ctx =>
        {
            var result = self(ctx);
            if (result.HasValue)
                return result;
        ...
        public static Pattern<Optional<T>, TContext> Optional<T, TContext>(this Pattern<T, TContext> self) => ctx =>
        {
            var result = self(ctx);
        ...
        public static Pattern<IEnumerable<T>, TContext> Flatten<T, TContext>(this Pattern<IEnumerable<IEnumerable<T>>, TContext> self) => ctx =>
        {
        ...
        public static Pattern<T, TContext> Where<T, TContext>(this Pattern<T, TContext> self, Func<T, bool> predicate) => ctx =>
        {
            var result = self(ctx);
            if (!result.HasValue)
                return result;
        ...
        public static Pattern<T, TContext> Unless<T, TContext>(this Pattern<T, TContext> self, Func<T, bool> predicate) => ctx =>
        {
            var result = self(ctx);
            if (!result.HasValue)
                return result;
